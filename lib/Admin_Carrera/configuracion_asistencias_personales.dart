...
class _ConfiguracionAsistenciaPersonalScreenState
  ...
  bool _ventanaHorariaActiva = false;
  DateTime? _ventanaInicio;
  ...
  void _initDesdeAsistencia(Map<String, dynamic> data) {
    _qrActivo = data['activo'] == true;
    _tiempoLimiteActivo = data['tiempoLimiteActivo'] == true;
    ...
    } else if (ae is String) {
      _activadoEn = DateTime.tryParse(ae);
    }
  ...
  String get _estadoEfectivo {
    ...
    if (_ventanaHorariaActiva && _ventanaInicio != null && _ventanaFin != null) {
      if (ahora.isBefore(_ventanaInicio!)) return 'programado';
      if (ahora.isAfter(_ventanaFin!)) return 'fuera_ventana';
    }
    if (_tiempoLimiteActivo && _activadoEn != null) {
      final expira = _activadoEn!.add(Duration(minutes: _tiempoLimiteMinutos));
      if (ahora.isAfter(expira)) return 'expirado';
    }
    return 'activo';
  }

  Duration? get _tiempoRestante {
    if (!_tiempoLimiteActivo || _activadoEn == null) return null;
    final expira = _activadoEn!.add(Duration(minutes: _tiempoLimiteMinutos));
    final r = expira.difference(DateTime.now());
    return r.isNegative ? Duration.zero : r;
  }

  Duration? get _tiempoParaInicio {
    if (!_ventanaHorariaActiva || _ventanaInicio == null) return null;
    final r = _ventanaInicio!.difference(DateTime.now());
  ...
  Future<void> _guardarConfiguracion() async {
    if (_ventanaHorariaActiva) {
      if (_ventanaInicio == null || _ventanaFin == null) {
        _snack('Define hora de inicio y fin de la ventana', error: true);
        return;
      }
      if (_ventanaFin!.isBefore(_ventanaInicio!)) {
    ...
    try {
      final updates = <String, dynamic>{
        'activo': _qrActivo,
        'tiempoLimiteActivo': _tiempoLimiteActivo,
        'tiempoLimiteMinutos': _tiempoLimiteMinutos,
        'ventanaHorariaActiva': _ventanaHorariaActiva,
        'ventanaInicio': (_ventanaHorariaActiva && _ventanaInicio != null)
            ? Timestamp.fromDate(_ventanaInicio!)
            : null,
        'ventanaFin': (_ventanaHorariaActiva && _ventanaFin != null)
            ? Timestamp.fromDate(_ventanaFin!)
            : null,
      };
  ...
  Widget _buildEstadoRealtime() {
    ...
    final restante = _tiempoRestante;
    final paraInicio = _tiempoParaInicio;
              ...
              children: [
                _buildConfigBloque(
                  ...
                  color: _qrActivo
                      ...
                      : const Color(0xFF90A4AE),
                  child: _buildToggle(
                    label: _qrActivo ? 'QR Activo' : 'QR Inactivo',
                    descripcion: _qrActivo
                    ...
                    if (_tiempoLimiteActivo) ...[
                      ...
                      Wrap(
                        ...
                        children: _opcionesMinutos.map((min) {
                          final sel = _tiempoLimiteMinutos == min;
                          return GestureDetector(
                            onTap: () =>
                                setState(() => _tiempoLimiteMinutos = min),
                      ...
                      Row(children: [
                        ...
                        Expanded(
                          child: Slider(
                            value: _tiempoLimiteMinutos.clamp(1, 120).toDouble(),
                            min: 1,
                            max: 120,
                            divisions: 119,
                            activeColor: Colors.orange.shade700,
                            onChanged: (v) => setState(
                                () => _tiempoLimiteMinutos = v.toInt()),
                          ),
                        ),
                      ...
                      Row(children: [
                        Expanded(
                          child: _buildSelectorHora(
                            label: 'INICIO',
                            hora: _ventanaInicio != null
                                ? TimeOfDay(
                                    hour: _ventanaInicio!.hour,
                                    minute: _ventanaInicio!.minute)
                                : null,
                            color: Colors.blue.shade700,
                            onTap: () async {
                              final picked = await showTimePicker(
                                context: context,
                                initialTime: _ventanaInicio != null
                                    ? TimeOfDay(
                                        hour: _ventanaInicio!.hour,
                                        minute: _ventanaInicio!.minute)
                                    : TimeOfDay.now(),
                              ...
                              if (picked != null && mounted) {
                                final ahora = DateTime.now();
                                setState(() => _ventanaInicio = DateTime(
                                    ahora.year,
                                    ahora.month,
                                    ahora.day,
                                    picked.hour,
                        ...
                        Expanded(
                          child: _buildSelectorHora(
                            label: 'FIN',
                            hora: _ventanaFin != null
                                ? TimeOfDay(
                                    hour: _ventanaFin!.hour,
                                    minute: _ventanaFin!.minute)
                                : null,
                            color: Colors.red.shade700,
                            onTap: () async {
                              final picked = await showTimePicker(
                                context: context,
                                initialTime: _ventanaFin != null
                                    ? TimeOfDay(
                                        hour: _ventanaFin!.hour,
                                        minute: _ventanaFin!.minute)
                                    : TimeOfDay.now(),
