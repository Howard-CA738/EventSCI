import 'package:flutter/material.dart';
  ...
  try {
    ...
    debugPrint('📊 ${asistenciasSnapshot.docs.length} documentos encontrados');
    ...
    // ✅ AGRUPAR por studentUsername para evitar duplicados
    ...
    for (final doc in asistenciasSnapshot.docs) {
      ...
      if (!estudiantesMap.containsKey(username)) {
        // Primer documento de este estudiante → crear entrada
        estudiantesMap[username] = {
          ...
          '_docIds': [doc.id],       // lista de IDs para buscar scans
          '_rawData': data,
        };
      } else {
        // Ya existe → solo agregar el docId extra
        (estudiantesMap[username]!['_docIds'] as List).add(doc.id);

        // Actualizar lastScan si este es más reciente
        final existingLast = estudiantesMap[username]!['lastScan'];
        final newLast = data['lastScan'];
        if (existingLast == null ||
            (newLast != null &&
                (newLast as Timestamp)
                    .compareTo(existingLast as Timestamp) > 0)) {
          estudiantesMap[username]!['lastScan'] = newLast;
        }
      }
    }

    // ✅ Ahora cargar scans y ciclo/grupo para cada estudiante único
    final futures = estudiantesMap.values.map((estudianteData) async {
      ...
      // Recolectar todos los scans de todos los docs de este estudiante
        ...
        try {
          ...
          if (carreraPath != null && username != null) {
            ...
            if (studentQuery.docs.isNotEmpty) {
              ...
              ciclo ??= studentDoc['ciclo'];
              grupo ??= studentDoc['grupo'];
            }
          }
        } catch (e) {
