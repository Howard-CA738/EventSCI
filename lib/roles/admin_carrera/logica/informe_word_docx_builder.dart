...
import '../datos/word_run.dart';
import 'informe_word_data_service.dart';

class InformeWordGenerator {
  static final InformeWordDataService _dataService = InformeWordDataService();
...
<w:tbl>
  ...
  static String _buildTablaResultados(List<FilaResultado> filas) {
    const int col1 = 1872;
    const int col2 = 1872;
    const int col3 = 1872;
    const int col4 = 1872;
...
<w:tbl>
  ...
  static String _buildTablaGanadores(
      String nombreEvento, List<FilaGanador> filas) {
...
<w:tbl>
  ...
  <w:tr>
    ...
    final Map<String, List<FilaGanador>> porCategoria = {};
  ...
  static Future<Uint8List> _obtenerBytes({
    ...
    final List<String> categorias =
        await _dataService.obtenerCategorias(eventId);
    final List<FilaResultado> filasResultado =
        await _dataService.obtenerFilasResultado(eventId);
    final Map<String, int> indicadores =
        await _dataService.obtenerIndicadores(eventId);
    final List<FilaGanador> filasGanadores =
        await _dataService.obtenerGanadores(
      eventId,
      filialId: filialId,
  ...
  static String _reemplazarTablasResultadosIndicadoresGanadores(
    String xml,
    List<FilaResultado> filasResultado,
    Map<String, int> indicadores,
    String nombreEvento,
    List<FilaGanador> filasGanadores,
  ) {
    final RegExp tablaRe = RegExp(r'<w:tbl\b.*?</w:tbl>', dotAll: true);
    ...
    for (int i = 0; i < matches.length; i++) {
      ...
      if (i == idxResultados) {
        ...
        if (pieMatchI != null && pieMatchI.group(0)!.contains('Tabla 0')) {
        ...
        sb.write('\n');
        sb.write(
            _buildPieTabla('Tabla 04. Resumen de ganadores de la Jornada'));
      }
    }
