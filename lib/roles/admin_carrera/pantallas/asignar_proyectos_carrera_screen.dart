...
import '/shared/logica/asignar_proyectos_service.dart';
import '/shared/logica/eventos_filtrados_service.dart';
import 'widgets/confirmar_asignacion_proyectos_dialog.dart';
...
class _AsignarProyectosCarreraScreenState
  ...
  final AsignarProyectosService _service = AsignarProyectosService();
  final EventosFiltradosService _eventosFiltradosService =
      EventosFiltradosService();
  ...
  Stream<QuerySnapshot> _buildEventsQuery() {
    return _eventosFiltradosService.eventosStream(
      filialId: _filialId,
      facultad: _facultad,
  ...
  Future<void> _cargarProyectosConRubrica(Rubrica rubrica) async {
    ...
    try {
      final resultado = await _service.cargarProyectosPorRubrica(
        eventoId: _eventoSeleccionado!,
        juradoId: _juradoSeleccionado!,
        rubrica: rubrica,
      );

      if (resultado.sinCategorias) {
        if (mounted) {
      ...
      if (mounted) {
        ...
        for (final p in resultado.proyectos) {
          if (p['yaAsignado'] == true) {
            _proyectosSeleccionados.add(p['id'] as String);
          }
        }
        setState(() {
          _proyectosDisponibles  = resultado.proyectos;
          _proyectosPorCategoria = resultado.proyectosPorCategoria;
  ...
  Future<void> _asignarProyectos() async {
    ...
    final confirmar = await mostrarConfirmarAsignacionProyectosDialog(
      context,
      modoEdicion: _modoEdicion,
      proyectosSeleccionados: _proyectosSeleccionados.length,
      proyectosAEliminar: proyectosAEliminar,
      nombreRubrica: _rubricaSeleccionada!.nombre,
      nombreJurado: _juradoData!['nombre'] as String,
    );
    ...
    try {
      final resultado = await _service.asignarProyectosPorBatch(
        proyectosDisponibles: _proyectosDisponibles,
        proyectosSeleccionados: _proyectosSeleccionados,
        juradoId: _juradoSeleccionado!,
      ...
      if (mounted) {
        final partes = <String>[];
        if (resultado.asignados   > 0) partes.add('${resultado.asignados} nuevo(s)');
        if (resultado.actualizados > 0) partes.add('${resultado.actualizados} actualizado(s)');
        if (resultado.eliminados  > 0) partes.add('${resultado.eliminados} eliminado(s)');
