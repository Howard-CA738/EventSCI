...
class _CrearListaJuradosScreenState extends State<CrearListaJuradosScreen> {
  ...
  Future<void> _crearLista() async {
    ...
    try {
      // Fix: antes siempre creaba un doc nuevo con .add(), sin verificar si ya
      // existía una lista de jurados para este filial/carrera/evento. Si
      // listas_certificados_importados_screen.dart ya había generado una
      // (o se usaba esta pantalla más de una vez), quedaban dos listas
      // paralelas y un jurado presente en ambas terminaba con dos
      // certificados distintos al "enviar". Ahora se busca y fusiona, igual
      // que ya hacía _generarListasJurados() en esa pantalla.
      final nuevosJurados = elegidos.map((j) => {
            'estudianteId': j.id,
            'nombre': j.nombre,
            'dni': j.dni,
      ...
      final existente = await FirebaseFirestore.instance
          ...
          .where('filialId', isEqualTo: _filialId)
          .where('carreraId', isEqualTo: _carreraId)
          .where('eventoId', isEqualTo: _evento!['id'])
          .limit(1)
          .get();
      ...
      if (existente.docs.isEmpty) {
        ...
        final actuales =
            List<Map<String, dynamic>>.from(doc.data()['estudiantes'] ?? []);
