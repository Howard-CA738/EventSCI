...
import '/roles/admin_carrera/datos/integrante_ref.dart';
...
class _EvaluacionFinalSuperAdminScreenState
  ...
  Future<void> _cargarConfig(String eventoId) async {
    try {
      ...
      setState(() {
        _config        = doc.exists ? EvalFinalConfig.fromMap(doc.data()!) : EvalFinalConfig();
        _configCargada = true;
      });
    } catch (e) {
      setState(() {
        _config        = EvalFinalConfig();
        _configCargada = true;
      });
    }
  }
  ...
  Future<void> _calcularNotas() async {
    ...
    try {
      ...
      final List<Future<NotaFinalItem>> futuros =
        ...
        if (!seleccionado) {
          if (_config.incluirDocenteNoSel) {
            ...
            // Fix: antes ignoraba pctAsistNoSel (usaba notaAsist sin ponderar).
            // Alineado con evaluacion_final_calculo.dart (ya migrado en Admin_Carrera)
            // para que ambos roles calculen la misma nota final con la misma config.
