...
class _ImportarCodigosCertificadoScreenState
  ...
  Future<(FirmanteResumen, FirmanteResumen, FirmanteResumen)> _cargarFirmantesParaEnvio() async {
    try {
      ...
      final facId   = generarFacultadId(_selectedFacultad ?? '');
      final decSnap = await db
          ...
          .get();
      return (
        FirmanteResumen.fromDoc(vSnap.exists ? vSnap.data() : null),
        FirmanteResumen.fromDoc(decSnap.exists ? decSnap.data() : null),
        FirmanteResumen.fromDoc(dSnap.exists ? dSnap.data() : null),
      );
    } catch (e) {
      debugPrint('Error cargando firmantes: $e');
      return (const FirmanteResumen(), const FirmanteResumen(), const FirmanteResumen());
    }
  }
  ...
  Future<void> _confirmarYGuardar() async {
    ...
    FirmanteResumen firma1 = const FirmanteResumen();
    FirmanteResumen firma2 = const FirmanteResumen();
    FirmanteResumen firma3 = const FirmanteResumen();
    String motivo = '';
    if (necesitaCompleto) {
