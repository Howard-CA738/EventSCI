...
import '../logica/editar_admin_service.dart';
...
class _EditarAdminScreenState extends State<EditarAdminScreen>
  ...
  final _service = EditarAdminService();
  ...
  Future<void> _loadAdminData() async {
    ...
    try {
      ...
      final data = await _service.cargarDatos(_adminUid!);
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      if (wantsPasswordChange || emailCambia) {
        ...
        await _service.reautenticar(user, _currentPasswordController.text);
        debugPrint('✅ Reautenticación exitosa');
      }

      if (wantsPasswordChange) {
        await _service.actualizarPassword(user, _newPasswordController.text);
        debugPrint('✅ Contraseña actualizada');
      }

      if (emailCambia) {
        await _service.actualizarEmail(user, nuevoEmail);
        _showSuccess(
          'Se envió un correo a $nuevoEmail para confirmar el cambio.',
        );
      }

      await _service.guardarDatos(
        uid: _adminUid!,
        nombre: _nameController.text.trim(),
        nuevoEmail: nuevoEmail,
        emailCambia: emailCambia,
      );
