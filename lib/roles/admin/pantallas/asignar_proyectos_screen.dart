...
import '/roles/admin/pantallas/crear_jurados_screen.dart';
...
class _AsignarProyectosScreenState extends State<AsignarProyectosScreen> {
  ...
  final AsignarProyectosService _asignarProyectosService =
      AsignarProyectosService();
  ...
  Future<void> _cargarProyectosConRubrica(Rubrica rubrica) async {
    ...
    try {
      final resultado = await _asignarProyectosService.cargarProyectosPorRubrica(
      ...
      if (mounted) {
        _proyectosSeleccionados.clear();
        for (var proyecto in resultado.proyectos) {
          if (proyecto['yaAsignado'] == true) {
            _proyectosSeleccionados.add(proyecto['id'].toString());
          }
        }
        ...
        if (resultado.proyectos.isEmpty && resultado.totalProyectosEnEvento > 0) {
          _mostrarSnackBar(
            'No hay proyectos de las categorías: ${resultado.categoriasJurado.join(", ")}',
            isWarning: true,
          );
        }
      }
  ...
  Future<void> _asignarProyectos() async {
    ...
    try {
      final resultado = await _asignarProyectosService.asignarProyectosPorBatch(
      ...
      if (mounted) {
        ...
        if (resultado.asignados > 0) {
          mensajes.add('${resultado.asignados} nuevo(s)');
        }
        if (resultado.actualizados > 0) {
          mensajes.add('${resultado.actualizados} actualizado(s)');
        }
        if (resultado.eliminados > 0) {
          mensajes.add('${resultado.eliminados} eliminado(s)');
        }
